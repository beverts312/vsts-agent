...
var sh = require('svchost'), shell = require('shelljs'), path = require('path');
...
env.getEnv(path.join(__dirname, '..', 'env.agent'), function (err, env) {
    if (err) {
        console.error(err);
        return;
    }
    console.log(JSON.stringify(env, null, 2));
    host.start(path.join(__dirname, 'vsoagent.js'), { args: process.argv.slice(2), env: env }, handleRestart);
});
