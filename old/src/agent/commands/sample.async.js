...
var SampleAsyncCommand = (function () {
    function SampleAsyncCommand(executionContext, command) {
        this.command = command;
        this.executionContext = executionContext;
        this.description = "Sample Async Command";
    }
    SampleAsyncCommand.prototype.runCommandAsync = function () {
        var _this = this;
        var defer = Q.defer();
        ...
        setTimeout(function () {
            _this.command.info('done running async command');
            // resolve or reject must get called!  In this sample, if you set result=fail, then it forces a failure
            if (_this.command.properties && _this.command.properties['result'] === 'fail') {
                ...
                defer.reject(new Error(_this.command.message));
                return;
            }
            else {
                defer.resolve(null);
...
exports.SampleAsyncCommand = SampleAsyncCommand;
