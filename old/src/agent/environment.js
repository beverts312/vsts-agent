...
function ensureEnvFile(envPath) {
    ...
    fs.exists(envPath, function (exists) {
        ...
        ;
        var vars = getFilteredEnv();
        var contents = "";
        for (var envvar in process.env) {
            contents += envvar + '=' + process.env[envvar] + os.EOL;
        }
        fs.writeFile(envPath, contents, 'utf8', function (err) {
            if (err) {
...
function getEnv(envPath, complete) {
    var env = process.env;
    fs.exists(envPath, function (exists) {
        if (exists) {
...
var resolveCapability = function (filteredEnv, tool, capability, valueIfMissing) {
    // Initialize
    var result = false;
    var toolName;
...
var resolveCapabilityViaShell = function (filteredEnv, command, args, capability) {
...
var setCapability = function (filteredEnv, name, val) {
...
function getCapabilities() {
    var filteredEnv = getFilteredEnv();
